...
export function setupSocketHandlers(io: Server): void {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('find_match', async (data: { address: string; username: string; stakeTier: number }) => {
      ...
      if (result.matched && result.opponent) {
        ...
        // Notify player 1
        io.to(game.player1.socketId).emit('match_found', {
          gameId: game.id, mySlot: 'player1',
          opponent: { username: game.player2.username, address: game.player2.address },
          ...
          terrain: game.terrain, wind: game.wind,
          p1x: game.player1.tankX, p2x: game.player2.tankX,
        });

        // Notify player 2
        io.to(game.player2.socketId).emit('match_found', {
          gameId: game.id, mySlot: 'player2',
          opponent: { username: game.player1.username, address: game.player1.address },
          tier: { name: tier.name, depositCents: tier.depositCents, baseCents: tier.baseCents },
          depositSats: game.depositSats, baseSats: game.baseSats,
          escrowAddress: escrowAddr, bsvPrice: game.bsvPriceAtStart,
          terrain: game.terrain, wind: game.wind,
    ...
    socket.on('fire_shot', (data: { angle: number; power: number }) => {
      ...
      const result = gameManager.fireShot(socket.id, data.angle, data.power);
      ...
      const shotData = {
        shooterSlot: result.shooterSlot,
        angle: result.angle,
        power: result.power,
        ...
        craterX: result.craterX,
        craterRadius: result.craterRadius,
        p1Hp: result.p1Hp,
        p2Hp: result.p2Hp,
        wind: result.wind,
        currentTurn: game?.currentTurn || 'player1',
        hitPayment: result.hitPayment || null,
        gameOver: result.gameOver,
      };

      if (game) {
        io.to(game.player1.socketId).emit('shot_result', shotData);
        io.to(game.player2.socketId).emit('shot_result', shotData);
      }
  ...
  async function handleGameEnd(game: any, result: GameOverResult) {
    ...
    const base = { winner: result.winner, reason: result.reason, pot: result.pot, settleTxid };
    ...
    if (result.winner) {
      ...
      const winnerName = result.winner === 'player1' ? p1.username : p2.username;
      ...
      io.to(loserSocket).emit('game_over', {
        ...base, payout: 0,
        message: result.reason === 'destroyed' ? 'Your tank was destroyed!'
      ...
      io.to(p1.socketId).emit('game_over', { ...base, payout: result.winnerPayout, message: 'Draw agreed! 🤝' });
      io.to(p2.socketId).emit('game_over', { ...base, payout: result.loserPayout, message: 'Draw agreed! 🤝' });
    }
    ...
    try {
      await db.recordGameEnd(
        ...
        { shotsFired: p1.shotsFired, damageDealt: p1.damageDealt },
