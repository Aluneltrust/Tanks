...
export class LobbyManager {
  ...
  setStatusByAddress(address: string, status: PlayerStatus): void {
    ...
    if (socketId) this.setStatus(socketId, status);
  }
  ...
  isOnline(address: string): boolean { return this.addressToSocket.has(address); }
  ...
  cancelChallengesFrom(socketId: string): void {
    for (const [id, ch] of this.challenges) {
      if (ch.fromSocketId === socketId) this.cancelChallenge(id);
    }
  }
